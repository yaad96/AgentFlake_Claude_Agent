...
def _read_wrapper_source(source_base, module, victim_pkg, wrapper_simple):
    """Read the auto-generated wrapper if present in Flaky/, else None."""
    rel_path = victim_pkg.replace(".", "/") + "/" + wrapper_simple + ".java"
    return find_source_file(source_base, module, rel_path)


def _synth_wrapper_template(victim_pkg, victim_class_simple, victim_method, wrapper_simple):
    """Fallback: synthesize the wrapper text matching run_nio_tracemop.sh's
    template, in case the file isn't on disk (e.g., post-cleanup re-run)."""
    return (
...
def _two_turn_protocol_lines():
    ...
    out.append("  - The most common need is FULL_FILE on the victim's source file, so you")
    out.append("    can see the inner @RunWith(JUnitQuickcheck.class) static classes whose")
    out.append("    static fields are the actual pollution sites.")
    out.append("  - If the victim spawns inner test classes via `testResult(Foo.class)`,")
    out.append("    ask for METHOD on those inner classes' property/test methods so you can")
    out.append("    see which static fields they mutate.")
...
def assemble_context_nio(result_container):
    ...
    out.append("  A Non-Idempotent-Outcome (NIO) test PASSES when run alone, but FAILS")
    out.append("  when run a SECOND TIME in the same JVM. The cause is self-pollution:")
    out.append("  the test mutates shared state (typically a `static` field — an `int`")
    out.append("  counter, a `Collection`, a `boolean` flag) that its second invocation")
    out.append("  then reads. The first run leaves residue; the second run sees that")
    out.append("  residue and its assertion fails (e.g. `expected:<100> but was:<200>`).")
    out.append("")
    out.append("  The canonical NIO fix is a CLEANUP STATEMENT appended to the END of")
    out.append("  the test method, after the existing assertion(s), that resets the")
    out.append("  polluted static field to its initial value. Examples seen in the")
    out.append("  literature and in this dataset:")
    out.append("     SomeClass.iterations = 0;        // reset a static int counter")
    out.append("     SomeClass.values.clear();        // empty a static List/Set")
    out.append("     SomeClass.flag = false;          // reset a static boolean")
    out.append("  Less commonly, the cleanup goes into a new @After method on the test")
    out.append("  class. Prefer the in-method cleanup unless multiple test methods in")
    ...
    if source_file:
        ...
        # For NIO we ALWAYS want the full class — the inner @RunWith static
        # classes (the ones spawned via `testResult(Foo.class)`) hold the
        # polluted static fields, and those live in the same .java file.
    ...
    out.append("")
    wrapper_path = _read_wrapper_source(source_base, module, victim_pkg, wrapper_simple)
    if wrapper_path:
        out.append(extract_full_class(wrapper_path) or "(wrapper file not readable)")
    else:
        out.append("(Wrapper file not found on disk — showing the canonical template:)")
        out.append("")
